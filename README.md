...
> 四步向导的前三步是**窄侧边栏**截图（并排展示），模板编辑器是整屏截图（单列展示）。

<p align="center">
  <img src="docs/screenshots/1-range.png" width="300" alt="① 选择记录范围：全部 / 视图筛选 / 手动勾选，并给出范围内记录数与预计产出" />
  <img src="docs/screenshots/2-template.png" width="300" alt="② 选择模板：当前数据表的模板列表、新建模板、导入/导出" />
  <img src="docs/screenshots/3-preview.png" width="300" alt="③ 打印预览：与打印共用同一套排版管线，所见即所得" />
</p>

**① 选范围 → ② 选模板 → ③ 预览**：支持全部记录 / 视图筛选 / 手动勾选；模板按数据表隔离存放；预览与打印**共用同一套排版管线**，所见即所得。

<img src="docs/screenshots/4-editor.png" width="100%" alt="模板编辑器：左侧字段与元素面板、中间纸张画布（含字段胶囊与版式区）、右侧属性面板" />

**模板编辑器**：拖拽摆放元素、字段绑定、版式区可视化（表头区 / 循环区 / 表尾区）、表格行列增删与单元格合并拆分、毫米级尺寸控制。
